...
export async function up(db) {
  ...
  await db.schema
    .alterTable('vehicle')
    .addForeignKeyConstraint('user_vehicle_fk', ['user_id'], 'user', [
      'user_id',
    ])
    .execute();
  ...
  await db.schema
    .alterTable('ticket')
    .addForeignKeyConstraint('ticket_user_fk', ['user_id'], 'user', ['user_id'])
    .execute();
  await db.schema
    .alterTable('ticket')
    .addForeignKeyConstraint('ticket_park_fk', ['park_id'], 'park', ['park_id'])
    .execute();

  await db.schema
    .alterTable('payment')
    .addForeignKeyConstraint('payment_ticket_fk', ['ticket_id'], 'ticket', [
      'ticket_id',
    ])
    .execute();
  ...
  await db.schema
    ...
    .addForeignKeyConstraint('violations_ticket_fk', ['ticket_id'], 'ticket', [
      'ticket_id',
    ])
    .execute();
  ...
  await db.schema
    .alterTable('park')
    .addForeignKeyConstraint('park_area_fk', ['area_id'], 'parking_area', [
      'area_id',
    ])
    .execute();
  await db.schema
    .alterTable('park')
    .addForeignKeyConstraint('park_device_fk', ['device_id'], 'device', [
      'device_id',
    ])
    .execute();
  ...
  await db.schema
    .alterTable('media')
    .addForeignKeyConstraint('media_device_fk', ['device_id'], 'device', [
      'device_id',
    ])
    .execute();
}
