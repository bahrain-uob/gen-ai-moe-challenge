...
export const WritingFeedbackContainer = ({
  ...
  if (feedback['Grammer Tool Feedback']) {
    grammarMistakes = feedback['Grammer Tool Feedback'].map((mistake, index) => {
      ...
      for (let x in mistake) {
        body.push(<p key={x}>{x.toString()}:  {JSON.stringify(mistake[x])}</p>);
      };
  ...
  return (
    <>
      ...
      {grammarMistakes ? grammarMistakes : <p className={pClassname}>No Mistakes Found</p>}
